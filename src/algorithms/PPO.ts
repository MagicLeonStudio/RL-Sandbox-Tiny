...
export class PPO {
  ...
  constructor(inputSize: number, actionSize: number, gamma = 0.99, epsilon = 0.2, epochs = 4) {
  ...
  train(states: number[][], actions: number[], rewards: number[]) {
    ...
    for (let e = 0; e < this.epochs; e++) {
      for (let t = 0; t < states.length; t++) {
        ...
        const oldLogProb = Math.log(probs[actions[t]] + 1e-10);
        const newProbs = this.actor.forward(states[t]);
        const newLogProb = Math.log(newProbs[actions[t]] + 1e-10);
        ...
        const actorLoss = -Math.min(ratio * adv, clippedRatio * adv);
        const criticLoss = (R - values[t]) ** 2;
